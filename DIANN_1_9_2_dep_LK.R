...
data = read.delim('R:/Group Vermeulen/Lila/Mass_spec_results/PBMCs_all/hela/report.pg_matrix - Copy.tsv', stringsAsFactors = F, sep = '\t') # = pg.matrix


# Convert columns that match the regex (basically the columns that contain maxLFQ intensities) to numeric
columns_to_convert <- grep("wt|V|B", names(data), value = TRUE)
...
LFQ_columns <- grep("wt|V|B", colnames(data_unique)) # get LFQ column numbers
...
data_folder <- "R:/Group Vermeulen/Lila/Mass_spec_results/PBMCs_all/hela"

write.table(data_results, file = paste0(data_folder, "/data_results_fdrtool_HeLa_DIANN_1.9.2.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
