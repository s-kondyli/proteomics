...
columns_to_convert <- grep("^[E|D]", names(data), value = TRUE)
...
LFQ_columns <- grep("^[E|D]", colnames(data_unique)) # get LFQ column numbers
...
plot_volcano(dep, contrast = "inhibitor_vs_DMSO", label_size = 2, add_names = TRUE, adjusted = T) +
  labs(
...
plot_single(dep, proteins = "LDLR", type = "centered")
...
write.table(data_results, file = paste0(data_folder, "/data_results_v16_.tsv"), sep = "\t", row.names = FALSE)
