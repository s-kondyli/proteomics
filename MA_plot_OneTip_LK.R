...
df <- iBAQ_info %>%
  ...
  mutate(
    Condition = case_when(
      Sample %in% colnames(iBAQ_info)[cols] ~ "V93_T_B", #condition name -> change each time
      TRUE ~ "other"
    )
  )
...
merged_df$Legend[merged_df$V93_T_vs_V93_B_p.adj < 0.05 & merged_df$V93_T_vs_V93_B_ratio > log2(1.5)] <- "Enriched in T cells" #for the proteins upregulated on the right -> you can rename it well
merged_df$Legend[merged_df$V93_T_vs_V93_B_p.adj < 0.05 & merged_df$V93_T_vs_V93_B_ratio < -log2(1.5)] <- "Enriched in B cells" #for the proteins downregulated on the left -> you can rename it well
...
ggplot(merged_df, aes(x = V93_T_B , y = V93_T_vs_V93_B_ratio, color = Legend)) + 
  geom_point() +  # Plot the points
  scale_color_manual(values = c("Enriched in T cells" = "#7b3294","Enriched in B cells"="#5ab4ac", "Background" = "azure3", "Hits"= "#ca0020")) + #manual color setting based on Legend column
  theme_minimal() +  # Use a minimal theme
  labs(x = "log2(mean_iBAQ_V88_T_B)", y = "log2(T/B)", title = "MA plot V93 T vs B cells") +# Labels and title
